...
app.delete("/deleteAutoSlots/:boardName", async (req, res) => {
  ...
  try {
    ...
    await connection.query(
    "DELETE FROM autoSlots WHERE board_id = ?",
    [boardId]
...
app.post("/quickMessages/saveAll", async (req, res) => {
    ...
    try {
        ...
        // Päivitä käyttäjän omat quickMessages-rivit
        const [rows] = await pool.query(
            `SELECT id
             FROM quickMessages
             WHERE board_id = ?
        ...
        if (rows.length !== quickMessages.length) {
